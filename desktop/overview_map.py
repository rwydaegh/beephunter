...
def main():
    Rf = newest("reference"); R = load(Rf)
    rt0, rt1 = R["tw"][0]/1000, R["tw"][-1]/1000
    cand = sorted(glob.glob(os.path.join(DL,"beephunter_hunter_*.json")), key=os.path.getmtime, reverse=True)
    Pf = None
    for h in cand:
        tw = np.array([x.get("tw") for x in json.load(open(h,encoding="utf-8"))["rows"]], float)/1000
        if np.isfinite(tw).any() and tw[np.isfinite(tw)][-1] > rt0 and tw[np.isfinite(tw)][0] < rt1:
            Pf = h; break
    if Pf is None: Pf = cand[0]
    P = load(Pf)
    dt = 0.2
    ...
    data = {"pts":pts,"home":list(HOME),"src":[round(slat,7),round(slon,7)],
            ...
            "dh":round(dh,0),"lops":lops,"dom":dom,
            "bbox":[[float(np.nanmin(latS)),float(np.nanmin(lonS))],[float(np.nanmax(latS)),float(np.nanmax(lonS))]]}
    html = TEMPLATE.replace("__DATA__", json.dumps(data))
    out = os.path.join(ROOT,"overview.html"); open(out,"w",encoding="utf-8").write(html)
    print("-"*64); print("saved", out)
...
const map=L.map('map',{zoomSnap:0,maxZoom:22,preferCanvas:true});
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxNativeZoom:19,maxZoom:22,attribution:'© OpenStreetMap'}).addTo(map);
...
L.marker(D.home).bindTooltip("HOME #24",{permanent:true,direction:"top"}).addTo(markers);
...
L.control.layers(baseColor,{"➤ walk direction (denser on zoom)":arrows,"⟂ abeam lines-of-position":lopL,
  "· raw GPS (jittery)":rawTrack,"⚑ home + source fit":markers},
